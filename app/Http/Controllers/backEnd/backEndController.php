...
class backEndController extends Controller
{
    ...
    public function login(REQUEST $request){
        ...
        }elseif(Auth::guard($request->get('guard'))->user()->is_labs == 1){
            return redirect()->route('xray_as_lab',Auth::guard($request->get('guard'))->user()->id);
        }elseif (Auth::guard($request->get('guard'))->user()->is_xray == 1) {
            return redirect()->route('labs_as_xray',Auth::guard($request->get('guard'))->user()->id);
